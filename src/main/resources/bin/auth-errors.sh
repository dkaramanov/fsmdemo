...
curl -H "Content-Type: application/json" -X POST -d '{"function":"getcontracts","from":"requester"}' http://localhost:${PORT}${URI}/____receiveMessage
echo -n ?; read x
...
echo -n ?; read x
curl -H "Content-Type: application/json" -X POST -d '{"function":"contracts","to":"requester"}' http://localhost:${PORT}${URI}/____sendMessage
