...
**/
package com.estafet.scribble.easyfsm.FSM;
...
public class FSM implements java.io.Serializable {
    ...
    {
    	Map m = null;
    	m = this._fsm.getCurrentState().getNewTransitionMap();
    	Collection values = m.values();
    	String listOfValues[] = new String[values.size()];
    	int i=0;
    	for (Iterator<FSMTransitionInfo> iter = values.iterator(); (iter.hasNext());)
    	{
    		FSMTransitionInfo info = iter.next();
    		listOfValues[i++] = info.getNextState();
    	}
    	return listOfValues;
    }
    public String[] getValidActions()
    {
    	...
    	{
    		FSMTransitionInfo info = iter.next();
    		listOfValues[i++] = info.getActionName();
    	}
    	return listOfValues;
    }
    public String[] getValidCommands()
    {
    	Map m = null;
    	Set s = null;
    	s = this._fsm.getCurrentState().getNewTransitionMap().keySet();
    	Object keys[] = s.toArray();
    	String listOfValues[] = new String[keys.length];
    	for (int i = 0; (i < keys.length); i++)
    	{
    		listOfValues[i] = keys[i].toString() + " ";
